...
`@references/scenario-design.md`의 워크플로우를 따라 검증할 시나리오를 도출한다. 직관·경험에 의존하면 핵심 케이스를 놓치고 의미 없는 케이스를 만든다.

도출 순서:
1. **Use Case** — 사용자 트랜잭션의 시작과 끝, main / alternative / exception flow 식별.
2. **입력 분석** — 각 입력에 대해 ECP(동치 분할) → BVA(경계값) 추가.
3. **다중 조건** — 조건 2개 이상이면 Decision Table.
4. **상태 흐름** — 흐름에 상태가 있으면 State Transition.
5. **조합 폭발** — 변수 4개 이상이면 Pairwise.
6. **불변식** — 순수 함수면 Property-Based 1~2개.
7. **엣지** — Error Guessing 체크리스트로 마지막 검토.

도출된 시나리오 목록을 사용자에게 한 줄씩 제시하고 (it/test 디스크립션 형태로 미리 보여줌), 추가/제외 의견을 받은 뒤 Step 5로 진행한다.
